...
//Fun��o que envia 1 byte direto do I2C para o chip
...
//Fun��o que envia 4 bits e d� um pulso no pino EN
...
// Fun��o que envia comandos (configura��es) de 8 bits para o LCD
void LCD_Command(uint8_t cmd){
	// 4 bits mais altos s�o enviados no modo COMANDO
	_LCD_Write_Half(cmd & 0xF0, LCD_COMMAND);
	// 4 bits mais baixos s�o jogados pra esquerda e enviados
	_LCD_Write_Half((cmd << 4) & 0xF0, LCD_COMMAND);
}

void LCD_Write_Char(char data){
	//4 bits mais altos s�o enviados no modo DADO
...
void LCD_Write_String(const char *str){
	...
	//La�o de percorrer a palavra at� \0
	for(i=0;str[i] != '\0';i++){
...
void LCD_Ponteiro(uint8_t linha, uint8_t coluna){
	...
	if(linha == 0){
		address = 0x80; // Seta o endere�o base como inicio da linha 1
	} else{
		address = 0xC0; //Seta o endere�o base como inicio da linha 2
	}
	LCD_Command(address+coluna); // soma endere�o base com coluna escolhida e envia o comando
}
